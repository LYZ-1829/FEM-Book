...
function printf(fid, varargin)
    fprintf(1, varargin{:});   % 屏幕
    fprintf(fid, varargin{:}); % 文件
end

% 辅助函数：输出矩阵到屏幕和文件（保留原 disp 格式）
...
function [L, dir_cos, Ke] = truss3d_element_stiffness(x1, x2, E, A)
    % 输入：节点坐标 x1, x2 (1x3向量), 弹性模量 E, 截面积 A
    % 输出：长度 L, 方向余弦 [cx,cy,cz], 6x6刚度矩阵 Ke
    delta = x2 - x1;
    L = norm(delta);
    if L < eps
...
function [epsilon, sigma, N] = truss3d_element_stress(x1, x2, E, A, de)
    % 输入：节点坐标 x1,x2, 弹性模量 E, 面积 A, 节点位移 de (6x1列向量)
    % 输出：应变 epsilon, 应力 sigma, 轴力 N
    [L, dir_cos, ~] = truss3d_element_stiffness(x1, x2, E, A);
    B = 1/L * [-dir_cos, dir_cos];  % 1x6 应变-位移矩阵
    de = de(:);
    epsilon = B * de;
    sigma = E * epsilon;
    N = sigma * A;
...
%% ----------------------------- 任务3：验证算例 ---------------------------------
printf(fid, '================== 三维杆单元程序验证 ==================\n\n');

% 算例1：一维杆单元 (沿x轴)
printf(fid, '------------------ 算例1：一维杆单元 ------------------\n');
x1 = [0,0,0]; x2 = [2,0,0];
E1 = 200e9; A1 = 1e-4;
de1 = [0,0,0, 1e-3,0,0]';

[L1, dc1, Ke1] = truss3d_element_stiffness(x1, x2, E1, A1);
printf(fid, '单元长度 L = %.6f m (理论 2.000 m)\n', L1);
printf(fid, '方向余弦 = (%.4f, %.4f, %.4f) (理论 1,0,0)\n', dc1(1), dc1(2), dc1(3));
printf(fid, '刚度矩阵 Ke1 (仅显示非零):\n');
print_matrix(fid, Ke1, 'Ke1');

[eps1, sig1, N1] = truss3d_element_stress(x1, x2, E1, A1, de1);
printf(fid, '应变 = %.4e (理论 5.000e-4)\n', eps1);
printf(fid, '应力 = %.2f MPa (理论 100.00 MPa)\n', sig1/1e6);
printf(fid, '轴力 = %.2f N (理论 10000.00 N)\n', N1);
...
printf(fid, '单元长度 L = %.6f m (理论 3.000 m)\n', L2);
printf(fid, '方向余弦 = (%.4f, %.4f, %.4f) (理论 1/3,2/3,2/3)\n', dc2(1), dc2(2), dc2(3));

[eps2, sig2, N2] = truss3d_element_stress(x1, x2, E2, A2, de2);
printf(fid, '应变 = %.4e (理论 1.000e-3)\n', eps2);
printf(fid, '应力 = %.2f MPa (理论 210.00 MPa)\n', sig2/1e6);
printf(fid, '轴力 = %.2f N (理论 42000.00 N)\n', N2);
...
Fe = Ke2 * de_unit;
printf(fid, '自由度 j=%d (节点1的z向位移) 为单位位移时，节点力列阵为 Ke 的第 %d 列：\n', j, j);
for i = 1:length(Fe)
    printf(fid, '  %12.6e\n', Fe(i));
end
printf(fid, 'Ke 的第 %d 列：\n', j);
for i = 1:6
    printf(fid, '  %12.6e\n', Ke2(i,j));
end
printf(fid, '解释：k_{ij} 表示第 j 个自由度产生单位位移时，在第 i 个自由度方向需施加的力。\n');
