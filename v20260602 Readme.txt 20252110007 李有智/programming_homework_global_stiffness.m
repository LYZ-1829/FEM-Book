...
%% --------------------------- 1. 前处理模块（示例数据）---------------------------
% 算例1：一维两单元杆结构（实际用二维框架处理，y自由度无约束）
function model = preprocess_example1()
    model.title = '一维两单元杆结构';
    model.nsd = 1;          % 空间维数（用于显示）
    model.ndof = 2;         % 每个节点自由度（u,v）
    model.nnp = 3;          % 节点数
    model.nel = 2;          % 单元数
    model.nen = 2;          % 每个单元节点数
    model.x = [0; 1; 2];
    model.y = [0; 0; 0];
    model.IEN = [1, 2; 2, 3];
    % 题目给出 EA/L = [100,200]，设 L1=1, L2=1，则 EA = [100,200]
    model.E = [100, 200];
    model.A = [1, 1];
    model.fixed_dof = [1, 2];      % 节点1固定 (u1=0, v1=0)
    model.fixed_val = [0, 0];
    model.force_dof = [5];          % 节点3水平力 (自由度5)
    model.force_val = [10];
...
function model = preprocess_example2()
    ...
    model.E = [1, 1];
    model.A = [1, 1];
    model.fixed_dof = [1,2,3,4];   % 节点1和2固定
    model.fixed_val = [0,0,0,0];
    model.force_dof = [5];          % 节点3水平力
    model.force_val = [10];
    model.n_dof = model.nnp * model.ndof;
end
...
%% --------------------------- 附加题：三维桁架求解 ---------------------------
fprintf(fid, '\n========== 附加题：三维空间桁架算例 ==========\n');
% 定义三维桁架：4个节点，3个杆件，节点1固定，节点4受垂直载荷
...
for e = 1:nel_3d
    ...
    [L, dir_cos, Ke] = truss3d_element_stiffness(x1, x2, E_3d, A_3d);
    Ke_list_3d{e} = Ke;
end
...
% 屏幕也显示关键结果（可选）
fprintf('程序运行完毕。请查看 results.txt 获取详细输出。\n');